...
export default function Sidebar({
  ...
  const renderCard = (card, depth=0) => {
    ...
    return (
      <div key={card.id}
        ...
        onClick={() => { if (!isRenaming) onOpenCard(card.id) }}
        ...
        style={{
          ...
          background: isOpen ? T.accentLight+'0.1)' : 'transparent',
      ...
      >
        ...
        {isRenaming ? (
          <input autoFocus value={renameDraft} onChange={e => setRenameDraft(e.target.value)}
            ...
            style={{ flex:1, background:'rgba(255,255,255,0.07)', border:`1px solid ${T.accentLight}0.35)`, borderRadius:4, padding:'1px 5px', color: T.textPrimary, fontSize:12, outline:'none', minWidth:0 }}
          />
        ) : (
          <span style={{ flex:1, fontSize:12.5, color:isOpen?T.textPrimary:T.textMuted, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', fontFamily: T.fontBody || "'DM Sans',sans-serif", letterSpacing:'0.01em' }}>
  ...
  return (
    <div style={{
      ...
      background: T.bgOverlay + '0.5)',
      backdropFilter:'blur(40px) saturate(1.6)',
      WebkitBackdropFilter:'blur(40px) saturate(1.6)',
      borderRadius:16,
      border:`1px solid ${T.borderColor}0.09)`,
    }}>

      {/* Header: search + filter + settings */}
      <div style={{ padding:'10px 10px 8px', display:'flex', gap:6, alignItems:'center' }}>
        <div style={{ position:'relative', flex:1 }}>
          <Icon name="search" size={11} style={{ position:'absolute', left:9, top:'50%', transform:'translateY(-50%)', color: T.textDarker, pointerEvents:'none' }} />
          <input
            value={search} onChange={e=>setSearch(e.target.value)}
            placeholder="Rechercher…"
            style={{ width:'100%', background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:8, padding:'6px 8px 6px 26px', color: T.textSecondary, fontSize:12, outline:'none', boxSizing:'border-box', transition:'border-color 0.15s' }}
            onFocus={e=>e.target.style.borderColor=T.accentLight+'0.3)'}
        ...
        <div ref={filterBtnRef}>
          <button onClick={()=>setShowFilter(v=>!v)} title="Filtrer & Trier"
            style={{ padding:'6px 7px', borderRadius:7, border: hasActiveFilter ? `1px solid ${T.accentLight}0.3)` : '1px solid transparent', background: hasActiveFilter ? T.accentLight+'0.1)' : 'rgba(255,255,255,0.04)', color: hasActiveFilter || showFilter ? T.accent : T.textDark, cursor:'pointer', transition:'color 0.12s', display:'flex', alignItems:'center', position:'relative' }}
            onMouseEnter={e=>{ if(!hasActiveFilter && !showFilter) e.currentTarget.style.color=T.accent }} onMouseLeave={e=>{ if(!hasActiveFilter && !showFilter) e.currentTarget.style.color=T.textDark }}>
            <Icon name="filter" size={13} />
            {hasActiveFilter && <div style={{ position:'absolute', top:3, right:3, width:5, height:5, borderRadius:'50%', background: T.accent }} />}
          </button>
        </div>
        {/* Settings */}
        <button onClick={onShowTypes} title="Types de cartes"
          style={{ padding:'6px 7px', borderRadius:7, border:showTypes?`1px solid ${T.accentLight}0.3)`:'1px solid transparent', background:showTypes?T.accentLight+'0.1)':'rgba(255,255,255,0.04)', color:showTypes?T.accent:T.textDark, cursor:'pointer', transition:'color 0.12s', display:'flex', alignItems:'center' }}
          onMouseEnter={e=>{ if(!showTypes) e.currentTarget.style.color=T.accent }} onMouseLeave={e=>{ if(!showTypes) e.currentTarget.style.color=T.textDark }}>
      ...
      >
        ...
        {cards.length===0 && !search && (
          <div style={{ padding:'24px 12px', textAlign:'center' }}>
            ...
            <p style={{ color: T.textDarker, fontSize:12, marginBottom:12 }}>Aucun document</p>
          </div>
        )}
        ...
        {!search && (
          <div
            ...
            style={{ display:'flex', alignItems:'center', gap:8, padding:'6px 10px', borderRadius:8, cursor:'pointer', color: T.textDarker, fontSize:12, marginTop:2, transition:'all 0.12s', fontFamily: T.fontBody || "'DM Sans',sans-serif" }}
            onMouseEnter={e => { e.currentTarget.style.background=T.accentLight+'0.07)'; e.currentTarget.style.color=T.accent }}
            onMouseLeave={e => { e.currentTarget.style.background='transparent'; e.currentTarget.style.color=T.textDarker }}
          >
      ...
      {/* Footer bar */}
      <div style={{ padding:'7px 8px', borderTop:`1px solid ${T.borderColor}0.06)`, display:'flex', gap:4, alignItems:'center' }}>
        {creatingFolder ? (
          <input
            ...
            style={{ flex:1, background:'rgba(255,255,255,0.06)', border:`1px solid ${T.accentLight}0.3)`, borderRadius:6, padding:'5px 8px', color: T.textPrimary, fontSize:12, outline:'none' }}
          />
        ) : (
          <>
            <FooterBtn icon="card" tooltip="Nouveau document" color={T.accent} filled onClick={()=>setShowNewCard(true)} />
            <FooterBtn icon="folder" tooltip="Nouveau dossier" color={T.accent} onClick={()=>setCreatingFolder(true)} />
            <div style={{ flex:1 }} />
            <FooterBtn emoji="🎨" tooltip="Canvas" color="#06b6d4" onClick={()=>onCreateCard('canvas')} />
            <FooterBtn emoji="🌳" tooltip="Arbre Généalogique" color="#a78bfa" onClick={()=>onCreateCard('family_tree')} />
            <FooterBtn emoji="🗺" tooltip="Carte Géographique" color="#f59e0b" onClick={()=>onCreateCard('geo_map')} />
          </>
        )}
      </div>
    </div>
...
function FilterRow({ icon, label, color, active, dot, hasChildren, onClick }) {
  const [hov, setHov] = useState(false)
  return (
    <div onClick={onClick}
      onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
