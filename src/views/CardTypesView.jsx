...
import { Icon, Btn, ColorPicker } from '../components/ui.jsx'
    ...
    <div style={{ display:'flex', height:'100%' }}>
      ...
      <div style={{ width:240, flexShrink:0, borderRight:'1px solid rgba(255,200,120,0.08)', display:'flex', flexDirection:'column' }}>
        <div style={{ padding:'10px 10px 8px', borderBottom:'1px solid rgba(255,255,255,0.05)', display:'flex', gap:7, alignItems:'center' }}>
          <div style={{ position:'relative', flex:1 }}>
            <Icon name="search" size={11} style={{ position:'absolute', left:8, top:'50%', transform:'translateY(-50%)', color:'#3a2a18' }} />
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Rechercher…"
              style={{ width:'100%', background:'rgba(255,255,255,0.04)', border:'none', borderRadius:6, padding:'5px 8px 5px 22px', color:'#c8b89a', fontSize:12, outline:'none' }} />
          </div>
          <button onClick={() => { setCreating(true); setSelected(null) }} style={{ background:'none', border:'1px solid rgba(255,255,255,0.09)', borderRadius:6, color:'#5a4a38', cursor:'pointer', padding:'4px 7px', fontSize:11 }}
        ...
        <div style={{ padding:'8px 12px', borderTop:'1px solid rgba(255,255,255,0.05)', fontSize:11, color:'#3a2a18' }}>
      ...
      <div style={{ flex:1, overflow:'auto', padding:'20px 24px' }}>
        {creating ? (
          ...
          <div style={{ textAlign:'center', paddingTop:60, color:'#3a2a18' }}>Sélectionnez un type</div>
        )}
      </div>
    </div>
  )
  ...
  return (
    <div>
      <div style={{ display:'flex', alignItems:'center', gap:6, padding:`5px 8px 5px ${8+depth*14}px`, borderRadius:6, cursor:'pointer', marginBottom:1, background: isSelected?'rgba(200,160,100,0.1)':'transparent', transition:'background 0.1s' }}
        ...
        {isOverridden && BUILTIN_TYPES.some(t=>t.id===type.id) && <span style={{ fontSize:9, color:'#c8a064', opacity:0.6, flexShrink:0 }}>✦</span>}
        {/* Expand arrow — RIGHT */}
        {!filtered && children.length > 0
          ? <span onClick={e => { e.stopPropagation(); setExpanded(!expanded) }} style={{ display:'flex', alignItems:'center', justifyContent:'center', color:'#4a3a28', flexShrink:0, width:16 }}>
    ...
    <div className="anim-fadeup">
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:22, paddingBottom:14, borderBottom:'1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          ...
          <h2 style={{ fontFamily:"'Lora',serif", fontSize:18, color:'#f0e6d3', fontWeight:500, margin:0 }}>{type.name}</h2>
          {isBuiltin && <span style={{ fontSize:10, color:'#5a4a38', background:'rgba(255,255,255,0.04)', padding:'2px 6px', borderRadius:4 }}>Intégré</span>}
        </div>
      ...
      <section style={{ marginBottom:26 }}>
        <h3 style={{ fontFamily:"'Lora',serif", fontSize:15, color:'#c8b89a', marginBottom:12, fontWeight:500 }}>Propriétés par défaut</h3>
        {props.length===0 && !addingProp && <p style={{ color:'#3a2a18', fontSize:12, marginBottom:10 }}>Aucune propriété</p>}
        ...
        {addingProp
          ...
          : <button onClick={() => setAddingProp(true)} style={{ display:'flex', alignItems:'center', gap:6, padding:'7px 11px', width:'100%', background:'rgba(255,255,255,0.02)', border:'1px dashed rgba(255,255,255,0.07)', borderRadius:7, color:'#4a3a28', fontSize:12, cursor:'pointer', transition:'all 0.1s' }}
      ...
      {/* Couleur */}
      <section>
        <h3 style={{ fontFamily:"'Lora',serif", fontSize:15, color:'#c8b89a', marginBottom:12, fontWeight:500 }}>Couleur</h3>
        <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:10 }}>
          <div style={{ width:32, height:32, borderRadius:8, background:color, border:'2px solid rgba(255,255,255,0.1)' }} />
          <span style={{ fontSize:12, color:'#5a4a38' }}>{color}</span>
        </div>
...
function PropRow({ prop, allTypes, onRename, onRemove }) {
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:8, padding:'7px 11px', background:'rgba(255,255,255,0.03)', borderRadius:7, border:'1px solid rgba(255,255,255,0.05)' }}>
      <span style={{ fontSize:10, color:'#4a3a28', background:'rgba(255,255,255,0.04)', padding:'1px 6px', borderRadius:3, flexShrink:0 }}>{badge}</span>
      {editing
        ? <input autoFocus value={draft} onChange={e=>setDraft(e.target.value)} onBlur={commit}
            onKeyDown={e => { if(e.key==='Enter') commit(); if(e.key==='Escape') { setDraft(prop.name); setEditing(false) } }}
            style={{ flex:1, background:'transparent', border:'none', borderBottom:'1px solid rgba(200,160,100,0.4)', color:'#e2d9c8', fontSize:12, outline:'none' }} />
        : <span onClick={() => setEditing(true)} title="Cliquer pour renommer"
            style={{ fontSize:12, flex:1, color:'#c8b89a', cursor:'text' }}>{prop.name}</span>
      }
      {targets && <span style={{ fontSize:10, color:'#5a4a38', flexShrink:0 }}>→ {targets}</span>}
      {prop.multiple && <span style={{ fontSize:9, color:'#4a3a28', flexShrink:0 }}>×n</span>}
      <button onClick={onRemove} style={{ background:'none', border:'none', color:'#3a2a18', cursor:'pointer', padding:'0 2px', fontSize:10, lineHeight:1, flexShrink:0 }}
...
export function DropdownPropPicker({ allTypes, onAdd, onCancel }) {
  ...
  const SCALARS = [
    { id:'__text',   icon:'T',  label:'Texte',     ft:'text',     color:'#9a8a70' },
    ...
    <div ref={ref} className="anim-slidedown" style={{ background:'rgba(8,4,0,0.97)', border:'1px solid rgba(200,160,100,0.18)', borderRadius:10, overflow:'hidden', boxShadow:'0 8px 32px rgba(0,0,0,0.7)', marginBottom:6 }}>
      <div style={{ padding:'8px 10px', borderBottom:'1px solid rgba(255,255,255,0.05)', display:'flex', alignItems:'center', gap:6 }}>
        <input autoFocus value={name} onChange={e=>setName(e.target.value)}
          ...
          style={{ flex:1, background:'transparent', border:'none', color:'#e2d9c8', fontSize:13, outline:'none' }} />
      </div>
      <div style={{ padding:'5px 10px', borderBottom:'1px solid rgba(255,255,255,0.04)', position:'relative' }}>
        <Icon name="search" size={11} style={{ position:'absolute', left:19, top:'50%', transform:'translateY(-50%)', color:'#3a2a18' }} />
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Filtrer les types…"
          style={{ width:'100%', background:'rgba(255,255,255,0.04)', border:'none', borderRadius:5, padding:'4px 8px 4px 22px', color:'#c8b89a', fontSize:11, outline:'none' }} />
      </div>
      <div style={{ padding:'4px 12px 2px', fontSize:10, color:'#3a2a18', textTransform:'uppercase', letterSpacing:'0.07em' }}>TYPE</div>
      <div style={{ maxHeight:280, overflowY:'auto' }}>
        {allRows.map(row => (
          <div key={row.id} onClick={() => doAdd(row)}
            ...
            <span style={{ fontSize:13, color:'#c8b89a', flex:1 }}>{row.label}</span>
            {row.isCard && <span style={{ fontSize:10, color:'#3a2a18' }}>🔗</span>}
          </div>
        ))}
        {allRows.length===0 && <div style={{ padding:'10px 12px', fontSize:12, color:'#3a2a18' }}>Aucun résultat</div>}
      </div>
      <div style={{ padding:'5px 12px', borderTop:'1px solid rgba(255,255,255,0.04)' }}>
        <button onClick={onCancel} style={{ background:'none', border:'none', color:'#4a3a28', cursor:'pointer', fontSize:11 }}>Annuler</button>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="anim-fadeup" style={{ maxWidth:560 }}>
      <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:24, paddingBottom:16, borderBottom:'1px solid rgba(255,255,255,0.06)' }}>
        <input value={icon} onChange={e=>setIcon(e.target.value)} style={{ width:50, background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.09)', borderRadius:8, padding:8, color:'#e2d9c8', fontSize:22, textAlign:'center', outline:'none', flexShrink:0 }} />
        <input autoFocus value={name} onChange={e=>setName(e.target.value)} placeholder="Nom du nouveau type…"
          ...
          style={{ flex:1, background:'transparent', border:'none', borderBottom:'1px solid rgba(200,160,100,0.3)', color:'#f0e6d3', fontSize:22, fontFamily:"'Lora',serif", fontWeight:600, outline:'none', paddingBottom:4 }} />
      </div>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:18, marginBottom:22 }}>
        <div>
          <div style={{ fontSize:11, color:'#4a3a28', marginBottom:8, textTransform:'uppercase', letterSpacing:'0.06em' }}>Sous-type de</div>
          <select value={parentId} onChange={e=>setParentId(e.target.value)}
            style={{ width:'100%', background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.09)', borderRadius:8, padding:'7px 10px', color:'#c8b89a', fontSize:12, outline:'none', cursor:'pointer' }}>
            <option value="">— Type racine —</option>
            {creatableTypes.map(t => <option key={t.id} value={t.id}>{t.icon} {t.name}</option>)}
          </select>
        </div>
        <div>
          <div style={{ fontSize:11, color:'#4a3a28', marginBottom:8, textTransform:'uppercase', letterSpacing:'0.06em' }}>Couleur</div>
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
