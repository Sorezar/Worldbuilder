...
export default function App() {
  ...
  return (
    <div style={{ height:'100vh', display:'flex', flexDirection:'column', overflow:'hidden', position:'relative' }}>
      {bg ? (
        <>
          <div style={{ position:'fixed', inset:0, zIndex:0, backgroundImage:`url(${bg})`, backgroundSize:'cover', backgroundPosition:'center', filter:'blur(8px) brightness(0.5)', transform:'scale(1.08)' }} />
          <div style={{ position:'fixed', inset:0, zIndex:0, background:'rgba(6,3,0,0.35)' }} />
        </>
      ) : (
              ...
              openCardIds.length === 0
                ...
                : <div style={{ flex:1, display:'flex', overflow:'hidden', minWidth:0, gap:8 }}>
                    {openCardIds.map((cid, i) => {
                      ...
                      if (!card) return null
                      return (
                        <div key={cid} style={{ flex:1, minWidth:300, maxWidth: openCardIds.length===1 ? '100%' : 560, height:'100%', overflow:'hidden', background:'rgba(10,6,1,0.55)', backdropFilter:'blur(40px) saturate(1.4)', WebkitBackdropFilter:'blur(40px) saturate(1.4)', borderRadius:16, border:'1px solid rgba(255,200,120,0.1)' }}>
...
function MondesHome({ world, setWorld, cards, allTypes, calendars, createCalendar, updateCalendar, deleteCalendar, onOpenCard, onCreateCard }) {
  ...
  return (
    <div style={{ flex:1, overflow:'auto', padding:'32px 40px', background:'rgba(8,4,0,0.5)', backdropFilter:'blur(40px) saturate(1.4)', WebkitBackdropFilter:'blur(40px) saturate(1.4)', borderRadius:16, border:'1px solid rgba(255,200,120,0.09)' }}>
...
function TopBar({ world, activeView, onSetView, cardCount }) {
  return (
    <div style={{ height:48, display:'flex', alignItems:'center', padding:'0 18px', background:'rgba(8,4,0,0.6)', backdropFilter:'blur(40px) saturate(1.6)', WebkitBackdropFilter:'blur(40px) saturate(1.6)', borderRadius:16, border:'1px solid rgba(255,200,120,0.09)', flexShrink:0, position:'relative' }}>
      <div style={{ display:'flex', alignItems:'center', gap:10, flexShrink:0 }}>
        <div style={{ width:26, height:26, borderRadius:8, background:`linear-gradient(135deg,${world.accentColor||'#c8a064'}55,${world.accentColor||'#c8a064'}18)`, border:`1px solid ${world.accentColor||'#c8a064'}28`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:11 }}>✦</div>
      ...
      <div style={{ position:'absolute', left:'50%', transform:'translateX(-50%)', display:'flex', gap:2, background:'rgba(255,255,255,0.05)', borderRadius:12, padding:'4px 5px', border:'1px solid rgba(255,255,255,0.07)' }}>
        {NAV.map(n => {
          ...
          return (
            <button key={n.id} onClick={() => onSetView(n.id)} style={{ display:'flex', alignItems:'center', gap:6, padding:'5px 13px', borderRadius:9, border:'none', background: active ? 'rgba(200,160,100,0.18)' : 'transparent', color: active ? '#c8a064' : '#4a3a28', fontSize:12, cursor:'pointer', transition:'all 0.12s' }}
